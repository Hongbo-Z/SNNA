...
class Model(nn.Module):
    ...
    def forward(self, x, register_hook=None, mask_token_idxs=None, attention_mask_idxs = None):
        x = x.unsqueeze(0).cuda() # (1, 3, w, h) add a batch dimension and send to GPU
        intermediate_output = self.backbone.get_intermediate_layers(x, n_last_blocks, register_hook=register_hook, mask_token_idxs=mask_token_idxs, attention_mask_idxs=attention_mask_idxs)
...
def generate_explanation(attribution_generator, image, class_index= None):
    ...
    return rawAttn, att_gradient, generic_att, norm_att, IGradient, SGradient_normAtt
...
for idx in random_idx:
    ...
    rawAttn, att_gradient, generic_att, norm_att, IGradient, SGradient_normAtt = generate_explanation(attribution_generator, img, class_index=top_pred_label)
    ...
    for num in mask_token_num[1:]: #exclude 0
        ...
        rawAttn_masked_pred = model(img, attention_mask_idxs=rawAttn_mask_token_idxs+1) # +1 for the [CLS] token
        ...
        att_gradient_masked_pred = model(img, attention_mask_idxs=att_gradient_mask_token_idxs+1)
        ...
        generic_att_masked_pred = model(img, attention_mask_idxs=generic_att_mask_token_idxs+1)
        ...
        norm_att_masked_pred = model(img, attention_mask_idxs=norm_att_mask_token_idxs+1)
        ...
        IGradient_masked_pred = model(img, attention_mask_idxs=IGradient_mask_token_idxs+1)
        ...
        SGradient_normAtt_masked_pred = model(img, attention_mask_idxs=SGradient_normAtt_mask_token_idxs+1)
